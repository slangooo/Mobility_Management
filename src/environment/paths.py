...
from src.environment import obstacles
from src.parameters import BUILDINGS_AREA_MARGIN
...
class Paths(object):
    ...
    def generate_paths(self, x_boundary=None, y_boundary=None):
        ...
        if not x_boundary or not y_boundary:
            max_x_boundary, max_y_boundary = np.max(self.obstacles_vertices, 0) + BUILDINGS_AREA_MARGIN
            min_x_boundary, min_y_boundary = np.min(self.obstacles_vertices, 0) - BUILDINGS_AREA_MARGIN

        else:
            min_x_boundary, max_x_boundary = x_boundary
            min_y_boundary, max_y_boundary = y_boundary

        self.bounds = np.array([[min_x_boundary, max_x_boundary], [min_y_boundary, max_y_boundary]])
...
if __name__ == '__main__':
    ...
    latitude = 40.418725331325
    longitude = -3.704271435627907
