from src.environment.paths import Paths
import numpy as np
import heapq
from src.data_structures import Coords3d
import matplotlib.pyplot as plt
from src.parameters import UE_HEIGHT
...
class Graph:
    ...
    def add_vertex(self, node_id, coords):
        ...
        new_vertex = Vertex(node_id, Coords3d(coords[0], coords[1], UE_HEIGHT))
...
def get_graph_from_segments(paths=None):
    ...
    for segment in paths.paths_segments:
        graph.add_edge(Coords3d(segment[0][0], segment[0][1], UE_HEIGHT), Coords3d(segment[1][0], segment[1][1], UE_HEIGHT),
                       float(np.linalg.norm(segment[1] - segment[0])))
    return graph, paths.bounds
