...
from src.ho_events import A3Event
...
class RfTransceiver:
    ...
    def get_serving_bs_info(self, recalculate=False):
        ...
        # Get index of base station that gives highest SINR
        ...
        serving_sinr = self.received_sinrs[self.serving_bs_idxs[0]][self.serving_bs_idxs[1]]

        for bs_type in range(len(self.stations_list)):
            for sinr_idx, received_sinr in enumerate(self.received_sinrs[bs_type]):
                if bs_type == self.serving_bs_idxs[0] and
        ...
        self.serving_bs_idxs = (max_idx, max_sinrs_idx)
